...
export function renderActivityLog() {
    elements.activityLog.innerHTML = state.activityLog.map(entry => {
...
export function updatePositions(traderPositions, userPositions, traderAccountData = null, userAccountData = null) {
...
export function updateActions(traderPositions) {
    const actionsHtml = traderPositions.length > 0
...
export function updateBalanceInfo(traderAccountData, userAccountData, traderTotalMargin, userBalance, dynamicSafetyBuffer, scalingFactor, userFreeBalance = null, traderFreeBalance = null) {
    // Trader balance info
    if (traderAccountData) {
        ...
        const traderAvailable = traderFreeBalance !== null ? traderFreeBalance : (traderTotal - traderMargin);
    ...
    const userMargin = userAccountData ? parseFloat(userAccountData.marginSummary.accountValue) - (userFreeBalance !== null ? userFreeBalance : 0) : 0;
